/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ClimateDataWhereInput } from "./ClimateDataWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ClimateDataOrderByInput } from "./ClimateDataOrderByInput";

@ArgsType()
class ClimateDataFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ClimateDataWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ClimateDataWhereInput, { nullable: true })
  @Type(() => ClimateDataWhereInput)
  where?: ClimateDataWhereInput;

  @ApiProperty({
    required: false,
    type: [ClimateDataOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ClimateDataOrderByInput], { nullable: true })
  @Type(() => ClimateDataOrderByInput)
  orderBy?: Array<ClimateDataOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ClimateDataFindManyArgs as ClimateDataFindManyArgs };
