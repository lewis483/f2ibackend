/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AnalysisReportWhereInput } from "./AnalysisReportWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { AnalysisReportOrderByInput } from "./AnalysisReportOrderByInput";

@ArgsType()
class AnalysisReportFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => AnalysisReportWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => AnalysisReportWhereInput, { nullable: true })
  @Type(() => AnalysisReportWhereInput)
  where?: AnalysisReportWhereInput;

  @ApiProperty({
    required: false,
    type: [AnalysisReportOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [AnalysisReportOrderByInput], { nullable: true })
  @Type(() => AnalysisReportOrderByInput)
  orderBy?: Array<AnalysisReportOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { AnalysisReportFindManyArgs as AnalysisReportFindManyArgs };
